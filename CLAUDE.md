...
- `skills/physics-tutor/SKILL.md` — 物理引导，锚定 2026 新高考物理（福建/浙江/江苏/北京等省自主命题）：力学综合/电磁/图像/实验，重过程与模型、反套路。viz=true（受力图 freebody / 运动图像 motion）。
- `skills/chem-tutor/SKILL.md` — 化学引导，锚定 2026 新高考化学（与物理同为省自主命题）：反应原理/电化学/平衡/工艺流程/有机/实验，重宏微符三重表征、反套路。viz=true（反应能量图 energy；分子3D/滴定曲线留后续）。
...
src/archive.ts    summarizeMistake(对话→错题字段) + toQuality(verdict→SM-2质量分)，CLI/review/server 共用
src/cli.ts        npm start —— 解新题，引导循环 + gaps 回注 + 错题归档
src/server.ts     npm run server —— Node http 服务(:8787)，SSE 流式，复用全部逻辑，桥接 Web 前端
...
src/db.test.ts    node:test 自检（db/evaluator/sm2/retry/subjects）
viewer/           Web app（Vite+React+R3F）。多视图：聊天/错题库/复习/历史。聊天走 SSE 流式 + 内联 Viz。
                  api.ts(SSE读取) / views/*(Chat,Mistakes,Review,History) / Viz 及各图组件。
                  可视化 spec 驱动：数学(solid/function)、物理(freebody/motion)、化学(energy)。
...
cd viewer && npm run dev        # 前端 :5173，proxy /api→:8787
```
打开 localhost:5173。生产：`viewer && npm run build` 后 server 自动静态托管 `viewer/dist`。
CLI（npm start/review/mistakes）仍可用，与 server 共用同一 data.db（别同时写）。
...
- viewer ↔ Tutor 集成：Web 模式下 spec 随 SSE `spec` 事件下发，聊天页直接内联 `<Viz>`。（旧 CLI 模式经 `viewer/public/spec.json` 文件 + 轮询的方式已被 Web 取代，但文件路仍可用。）`Spec` 类型在 `src/vizspec.ts` 与 `viewer/src/spec.ts` 各存一份（无共享构建）。
- 服务流式：`ask(agent, text, {onDelta})` 临时订阅 text_delta；CLI 推 stdout，server 推 SSE `delta`。同一 `ask`，两种消费。
